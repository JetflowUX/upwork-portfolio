...
export function Sidebar({ mobileOpen = false, onClose }: { mobileOpen?: boolean; onClose?: () => void; }) {
  const base = 'w-64 bg-ide-sidebar border-r border-ide-border flex flex-col shrink-0';
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 block ${base} md:hidden`}>
      <div className="h-10 flex items-center justify-between px-3 border-b border-ide-border">
        ...
        <button onClick={onClose} className="p-1 rounded hover:bg-ide-highlight">
          <X size={16} />
        </button>
      </div>
      <SidebarContent />
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col mt-1">
          <FileItem name="home.tsx" icon={<FileCode2 size={16} className="text-[#519aba]" />} active />
          <FileItem name="about.html" icon={<FileCode2 size={16} className="text-[#e34c26]" />} />
          <FileItem name="projects.js" icon={<FileCode2 size={16} className="text-[#f1e05a]" />} />
          <FileItem name="skills.json" icon={<FileJson size={16} className="text-[#cbcb41]" />} />
          <FileItem name="experience.ts" icon={<FileType2 size={16} className="text-[#3178c6]" />} />
          <FileItem name="contact.css" icon={<FileCode2 size={16} className="text-[#563d7c]" />} />
          <FileItem name="README.md" icon={<FileText size={16} className="text-[#083fa1]" />} />
          <FileItem name="Resume.pdf" icon={<FileImage size={16} className="text-[#b02626]" />} />
        </div>
      </div>
      ...
      <div className="p-4 mt-auto">
        <div className="flex items-center justify-between bg-[#1e1e2e] border border-[#313244] rounded-md px-3 py-2 cursor-pointer hover:bg-[#2a2b3d] transition-colors">
          ...
          <span className="text-[10px] text-ide-muted bg-black/30 px-1.5 py-0.5 rounded">AI</span>
        </div>
      </div>
    </>
  );
...
}: {name: string;icon: React.ReactNode;active?: boolean;}) {
  return (
    <div
